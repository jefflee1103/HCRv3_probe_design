...
fasta_file <- "./input/Dmel_pros-RL_exon_cleaned.fa" 
...
pass_a_stack      <- TRUE
pass_c_stack      <- TRUE
pass_c_spec_stack <- TRUE # stringent one
...
plot_inspection(candidate_probes_filtered, candidate_probes, "Tm")
...
```{r}
## ---- Set params
blast_file <- "~/Documents/Data/BLAST/Dmel/Dmel_BLASTdb_ens99.fa"
...
plot_inspection(candidate_probes_screened, candidate_probes, "Tm") +
...
## ---- Set params
probe_spacing <- 3
max_probe_pairs <- 30 # 33 is max pair count for IDT oPools cheap configuration
...
## ---- Inspect
plot_inspection(candidate_probes_final, candidate_probes, "Tm") +
  geom_linerange(data = merge_df, 
      aes(x = "", ymin = start, ymax = end),
      colour = "gray40", size = 2,
      inherit.aes = FALSE) + 
